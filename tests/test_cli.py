...
def test_interactive_shows_menu():
    runner = CliRunner(env={"JAM_HOME": "/tmp/dev"})
    # Pick list (3), answer "no" to show descriptions
    result = runner.invoke(main, [], input="3\nn\n")
    assert "pick a command" in result.output
    assert "new" in result.output
    assert "list" in result.output


def test_interactive_list(tmp_path):
    repo = tmp_path / "coolrepo" / ".git"
    repo.mkdir(parents=True)
    runner = CliRunner(env={"JAM_HOME": str(tmp_path)})
    # Pick list (3), no descriptions
    result = runner.invoke(main, [], input="3\nn\n")
    assert result.exit_code == 0
    assert "coolrepo" in result.output


def test_interactive_list_with_info(tmp_path):
    repo = tmp_path / "coolrepo" / ".git"
    repo.mkdir(parents=True)
    readme = tmp_path / "coolrepo" / "README.md"
    readme.write_text("# coolrepo\n\nsuper cool project\n")
    runner = CliRunner(env={"JAM_HOME": str(tmp_path)})
    # Pick list (3), yes to descriptions
    result = runner.invoke(main, [], input="3\ny\n")
    assert result.exit_code == 0
    assert "super cool project" in result.output


@patch("jam.cli.run")
@patch("os.path.isdir", return_value=True)
def test_interactive_down(mock_isdir, mock_run):
    mock_run.side_effect = [ok()]
    runner = CliRunner(env={"JAM_HOME": "/tmp/dev"})
    # Pick down (5), repo name "myrepo", no force
    result = runner.invoke(main, [], input="5\nmyrepo\nn\n")
    assert result.exit_code == 0
    assert "Pulled" in result.output
...
@patch("jam.cli.run")
@patch("os.path.isdir", return_value=True)
def test_interactive_up(mock_isdir, mock_run):
    mock_run.side_effect = [ok(), ok(), ok()]
    runner = CliRunner(env={"JAM_HOME": "/tmp/dev"})
    # Pick up (4), repo name "myrepo", message "hello", no force
    result = runner.invoke(main, [], input="4\nmyrepo\nhello\nn\n")
    assert result.exit_code == 0
    assert "Pushed" in result.output
    mock_run.assert_any_call('git commit -m "hello"', cwd="/tmp/dev/myrepo")
