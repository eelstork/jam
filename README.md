...
| 🔺`jam allow-all [REPO]` | Grant Claude Code broad tool permissions across all repos, or just `REPO` to try it on one first. Merges a permissive `permissions.allow` list (Bash, Read, Edit, Write, WebFetch, Agent, the GitHub MCP server, and friends) into each repo's `.claude/settings.json`, then commits and pushes. Existing settings and allow entries are preserved. Per repo: skips unless on `main`, pulls (`--ff-only`), edits, commits only that file, then pushes; repos that are off `main`, behind with conflicts, or carrying unpushed commits are skipped with a one-line reason. `--unset` removes the rules. [allow_all.py](src/jam/commands/allow_all.py) |
