...
COMMAND_INFO = [
    ...
    ("clone", "Clone a repo as a new repo"),
    ("list", "List repos"),
    ("up", "Add all, commit, and push"),
    ("down", "Pull latest changes"),
    ("land", "Merge latest branch into main"),
    ("infuse", "Copy files from one repo into another"),
    ("delete", "Delete a repo locally and on GitHub"),
]


def interactive_mode(ctx):
    """Guide the user through picking and running a command."""
    click.echo("jam - pick a command:\n")
    for i, (name, desc) in enumerate(COMMAND_INFO, 1):
        click.echo(f"  {i}. {name:8s} {desc}")
    click.echo()

    choice = click.prompt("Command number", type=click.IntRange(1, len(COMMAND_INFO)))
    cmd_name, _ = COMMAND_INFO[choice - 1]
    click.echo()
    ...
    if cmd_name == "new":
        ...
        public = click.confirm("Public?", default=False)
        args = [cmd_name, name]
        if desc:
            args.append(desc)
        if public:
        ...
        source = click.prompt("Source repo")
        target = click.prompt("Target name")
        desc = click.prompt("Description", default="")
        public = click.confirm("Public?", default=False)
        args = [cmd_name, source, target]
        if desc:
            args.append(desc)
        if public:
            args.append("--public")
        ...
        info = click.confirm("Show descriptions?", default=False)
        args = [cmd_name]
        if info:
            args.append("--info")

    elif cmd_name == "up":
        name = click.prompt("Repo name (leave blank for current dir)", default="")
        message = click.prompt("Commit message")
        force = click.confirm("Force push?", default=False)
        args = [cmd_name]
        if force:
            args.append("--force")
        if name:
        ...
        name = click.prompt("Repo name (leave blank for current dir)", default="")
        force = click.confirm("Force pull (discard local changes)?", default=False)
        args = [cmd_name]
        if force:
            args.append("--force")
        if name:
        ...
        name = click.prompt("Repo name (leave blank for current dir)", default="")
        fast = click.confirm("Land without confirmation?", default=False)
        show_all = False
        if not fast:
            show_all = click.confirm("Show all commits?", default=False)
        args = [cmd_name]
        if fast:
            args.append("--fast")
        if show_all:
    ...
    elif cmd_name == "infuse":
        source = click.prompt("Source repo")
        target = click.prompt("Target (or leave blank if inside target repo)", default="")
        args = [cmd_name, source]
        if target:
    ...
    click.echo()
    # Look up the subcommand and invoke it
